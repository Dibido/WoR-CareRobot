...
/* virtual */ void AutomatedGuidedVehiclePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr sdf)
{
  if (!ros::isInitialized())
  {
    int argc = 0;
    char** argv = nullptr;
    ros::init(argc, argv, "AGV_model_plugin", ros::init_options::NoSigintHandler);
  }
  ...
  rosSubPath = rosNode->subscribe(AGV_PATH_TOPIC, 1, &AutomatedGuidedVehiclePlugin::setPathCallBack, this);
  rosSubSpeed = rosNode->subscribe(AGV_PATH_SPEED, 1, &AutomatedGuidedVehiclePlugin::setSpeedCallBack, this);
  ROS_INFO("TOPICS CREATED");

  // Start updating the plugin every itteration of the simulation
  updateConnection = event::Events::ConnectWorldUpdateBegin(std::bind(&AutomatedGuidedVehiclePlugin::onUpdate, this));

  ROS_INFO("\nThe AutomatedGuideVehiclePlugin is attach to model: %s", model->GetName().c_str());
}
...
void AutomatedGuidedVehiclePlugin::onUpdate()
{
  ...
  if (movingForward)
  {
    this->model->SetLinearVel(ignition::math::Vector3d(speedX, speedY, speedZ));
  }
  else
  {
    this->model->SetLinearVel(ignition::math::Vector3d(-speedX, -speedY, -speedZ));
  ...
  if (directionChangeInterval < (currentTime - lastDirectionChangeInterval).Double())
  {
...
{
  // Checking Elements that are relevant for the plugin and shutdown if some are missing
  if (!checkSDFElements(sdf))
  {
    ROS_WARN("****************************\nOne or more elements are missing. Stopping "
             "launch\n****************************\n");
    gazebo::shutdown();
  }

  // Setting variables
  double directionChangeIntervalSDF = sdf->Get<double>("directionChangeInterval");

  // Parse the starting and ending position
  Position startingPosition = parsePosition(sdf->Get<std::string>("startPose"));
...
void AutomatedGuidedVehiclePlugin::calculateAGVSpeed(const Position& startPosition, const Position& endPosition)
{
  ...
  ROS_DEBUG("Diff X: %f \nDiff Y: %f\nDiff Z: %f\nChangeInterval: %f", differenceX, differenceY, differenceZ,
...
Position AutomatedGuidedVehiclePlugin::parsePosition(const std::string& stringPosition)
{
...
bool AutomatedGuidedVehiclePlugin::checkSDFElements(const sdf::ElementPtr& sdf)
{
  bool allElementsPresent = true;

  if (!sdf->HasElement("directionChangeInterval"))
  {
    ROS_WARN("SDF Error: directionChangeInterval tag is missing in the sdf file\n");
...
void AutomatedGuidedVehiclePlugin::setPathCallBack(const sim_agv::agv_path& msg)
{
...
void AutomatedGuidedVehiclePlugin::setAGVPath(double givenDirectionChangeInterval, const Position& startingPosition,
                                              const Position& endingPosition)
{
  ...
  ignition::math::Pose3<double> pose(startPos.x, startPos.y, startPos.z, 0.0, 0.0, 0.0);
...
void AutomatedGuidedVehiclePlugin::setSpeedCallBack(const sim_agv::agv_speed& msg)
{
  double elapsedTime = (world->SimTime() - lastDirectionChangeInterval).Double();
  double percentageLeft = elapsedTime / directionChangeInterval;

  lastDirectionChangeInterval = world->SimTime().Double() - (percentageLeft * msg.changeDirectionInterval);
