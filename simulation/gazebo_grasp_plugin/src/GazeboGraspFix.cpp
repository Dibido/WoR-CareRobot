...
void GazeboGraspFix::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  {
    std::cout << "GazeboGraspFix: Using default "
  ...
  else
  {
    ...
    std::cout << "GazeboGraspFix: Using disable_collisions_on_attach "
  ...
  {
    std::cout << "GazeboGraspFix: Using default tolerance of "
  ...
  if (!updateRateElem)
  {
    std::cout << "GazeboGraspFix: Using  " << DEFAULT_UPDATE_RATE
  ...
  if (!maxGripCountElem)
  {
    std::cout << "GazeboGraspFix: Using  " << DEFAULT_MAX_GRIP_COUNT
  ...
  else
  {
    ...
    std::cout << "GazeboGraspFix: Using max_grip_count " << this->maxGripCount
              << std::endl;
  }
  ...
  if (!gripCountThresholdElem)
  {
    ...
    std::cout << "GazeboGraspFix: Using  " << this->gripCountThreshold
  ...
  else
  {
    ...
    std::cout << "GazeboGraspFix: Using grip_count_threshold "
  ...
  {
    std::cout << "GazeboGraspFix: Using  " << DEFAULT_RELEASE_TOLERANCE
  ...
  else
  {
    ...
    std::cout << "GazeboGraspFix: Using release_tolerance "
  ...
  for (; armElem != NULL; armElem = armElem->GetNextElement("arm"))
  {
         ...
         collIt != _collisions.end(); ++collIt)
    {
      ...
      std::cout << "GazeboGraspFix: Adding collision scoped name " << collName
                << std::endl;
    ...
    std::string& attachedToGripper) const
{
  ...
  // std::cout <<"Gripper "<<gripperName<<" is involved in the
  // grasp"<<std::endl;
  if (gripper.isObjectAttached())
  {
...
void GazeboGraspFix::OnUpdate()
{
  ...
  for (objIt = contPoints.begin(); objIt != contPoints.end(); ++objIt)
  {
    ...
    for (lIt = objIt->second.begin(); lIt != objIt->second.end(); ++lIt)
    {
      ...
      // std::cout  << "Found collision with "<<linkName<<": "<<avgForce.x<<",
       ...
       ocIt != objectContactInfo.end(); ++ocIt)
  {
    ...
    // std::cout <<"Number applied forces on "<<objName<<":
    ...
    // std::cout <<"Grasp Held: "<<objName<<" grip count:
    ...
    // std::cout <<"GRIPPING "<<objName<<", grip count "<<counts<<" (threshold
    // "<<this->gripCountThreshold<<")"<<std::endl;
    ...
    // std::cout <<"GazeboGraspFix: Attaching "<<objName<<" to gripper
         ...
         contPointsIt != contPointsTmp.end(); ++contPointsIt)
    {
      ...
      // std::cout <<"Checking initial contact with "<<collidingLink<<" and
      // "<<graspingGripperName<<std::endl;
       ...
       gripCntIt != this->gripCounts.end(); ++gripCntIt)
  {
    ...
    // std::cout <<"NOT-GRIPPING "<<objName<<", grip count
    // "<<gripCntIt->second<<" (threshold
    // "<<this->gripCountThreshold<<")"<<std::endl;
    ...
    for (pointIt = initColls.begin(); pointIt != initColls.end(); ++pointIt)
    {
      ...
      // std::cout <<"Obj Trans "<<cpInfo.collLink->GetName()<<":
      // "<<relObjPos.x<<", "<<relObjPos.y<<",
      // "<<relObjPos.z<<std::endl; std::cout <<"Cont Trans
      ...
      // std::cout <<"Diff for link "<<cpInfo.collLink->GetName()<<":
      // "<<diff<<std::endl;
    ...
    { // sufficient links did not meet the criteria to be close enough to the
      ...
      // std::cout <<"GazeboGraspFix: Detaching "<<objName<<" from gripper
      // "<<graspingGripperName<<"!!!!!!!"<<std::endl;
    ...
    float lengthRatio)
{
  ...
  for (it1 = forces.begin(); it1 != forces.end(); ++it1)
  {
    ...
    for (it2 = it1 + 1; it2 != forces.end(); ++it2)
    {
      ...
      // std::cout <<"Angular distance between v1.len="<<v1.GetLength()<<" and
      // v2.len="<<v2.GetLength()<<":
...
void GazeboGraspFix::OnContact(const ConstContactsPtr& _msg)
{
  ...
  for (int i = 0; i < _msg->contact_size(); ++i)
  {
        ...
        (collision2 && !collision2->IsStatic()))
    {
      ...
      std::cout <<"SIMPLE trans:
      "<<sDiff.x<<","<<sDiff.y<<","<<sDiff.z<<std::endl; std::cout <<"coll world
      pose: "<<linkWorldPose.pos.x<<", "<<linkWorldPose.pos.y<<",
      "<<linkWorldPose.pos.z<<std::endl; std::cout <<"contact avg pose:
      "<<avgPos.x<<", "<<avgPos.y<<",
      "<<avgPos.z<<std::endl;
      ...
      // now, get the pose of the object and compute it's relative position to
      ...
      // std::cout <<"Average force of contact= "<<avgForce.x<<",
      // "<<avgForce.y<<", "<<avgForce.z<<" out of
