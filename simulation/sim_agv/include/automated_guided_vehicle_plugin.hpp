...
public:
  ...
  /*
   * @author Wouter van Uum
   * @brief Called every itteration of the simulation. Contains functions that are used to move the AGV.
   */
  void onUpdate();
...
private:
   ...
   * @param sdf: The sdf file which contains the different elements that need to be parsed
   */
  ...
  /*
   * @author Wouter van Uum
   * @brief Used to calculate the x, y and z speed of the AGV. It uses the directionChangeInterval and the two given
   * positions to calculate the speed.
   * @param startingPosition: The start position of the AGV
   * @param endingPosition: The end position of the AGV
   */
  void calculateAGVSpeed(const Position& startingPosition, const Position& endingPosition);

  /*
   * @author Wouter van Uum
   * @brief Used to check if all elemets that are required for the AGV are present in the SDF file.
  ...
  /*
   * @author Wouter van Uum
   * @brief used to set the speed, starting and ending position of the AGV. Sets the AGV to the startPosition so that
   *when the simulation starts (or is going on) the AGV will start at its starting position and move accordingly.
   * @param givenDirectionChangeInterval: The amount of seconds the AGV has to travel between the two points
   *	 startingPosition: The start position of the AGV
   *	 endingPositon: The end position of the AGV
   */
  void setAGVPath(double givenDirectionChangeInterval, const Position& startingPosition,
   ...
   * @param msg: An agv_path message which containts the direcionChangeInterval, start- and endingposition.
   */
  void setPathCallBack(const sim_agv::agv_path& msg);

  /*
   * @author Wouter van Uum
   * @brief Used by a ros topic to set the speed of the AGV. It changes the direciontChangeInterval and calculates the
   * speeds with the new direciontChangeInterval.
   * @param msg: an agv_speed message which contains the directionChangeInterval.
   */
