 ...
 *
 * typename T: T must be an arithmetic type, i.e. an integral or floating point
 * type const std::size_t M: rows, number of rows of the matrix const
 * std::size_t N: columns, number of columns of the matrix
 */
template <typename T,
...
class Matrix
{
    public:
  /**
   * @name Compile-time assertion checking: see
  ...
  static_assert(std::is_arithmetic<T>::value,
                "Value T must be arithmetic, see "
                "http://en.cppreference.com/w/cpp/types/is_arithmetic");
  /**
   *
   */
  static_assert(M > 0 && N > 0,
                "M (rows) and N (columns) must both be greater than 0");
  //@}
  /**
  ...
  /**
   * Ctor with a list of lists of values where aList must contain M
   * elements and each list in aList must contain N elements
   */
   ...
   * If aRowIndex > getRows() an exception of type std::out_of_range is
   * thrown.
   */
   ...
   * If aRowIndex > getRows() or aColumnIndex > getColumns an exception
  ...
  /**
   * Returns the inverse of the matrix, assuming it's inverseable.
   * @see https://en.wikipedia.org/wiki/Invertible_matrix
   */
  Matrix<T, M, N> inverse() const;
  /**
   * Returns the right pseudo inverse of the matrix.
  ...
  //@{
  /**
   * @return a string representation of the matrix for printing on the
   * screen
   */
...
/**
 * Compare two matrices using a Precision and a factor. The actual used
 * precision is aPrecission*aFactor. If all rows are equal according this
 * aPrecission*aFactor true is returned, false otherwise
 */
