...
namespace rp
{
  namespace net
  {
    ...
    SocketAddress::SocketAddress(const char* addrString,
                                 int port,
    ...
    u_result
        SocketAddress::setAddressFromString(const char* address_string,
                                            SocketAddress::address_type_t type)
    {
      int ans = 0;
    ...
    size_t
        SocketAddress::LoopUpHostName(const char* hostname,
    ...
    namespace net
    {
      ...
      class _single_thread StreamSocketImpl : public StreamSocket
      {
        ...
        virtual u_result setTimeout(_u32 timeout, socket_direction_mask msk)
        {
          ...
          {
            ans = ::setsockopt(_socket_fd, SOL_SOCKET, SO_RCVTIMEO,
                               ( char* )&tv, ( int )sizeof(tv));
            if (ans)
              return RESULT_OPERATION_FAIL;
          }
          ...
          {
            ans = ::setsockopt(_socket_fd, SOL_SOCKET, SO_SNDTIMEO,
                               ( char* )&tv, ( int )sizeof(tv));
            if (ans)
              return RESULT_OPERATION_FAIL;
          }
        ...
        virtual u_result connect(const SocketAddress& pairAddress)
        {
          ...
          int ans =
              ::connect(_socket_fd, addr, ( int )sizeof(sockaddr_storage));
          if (!ans)
            return RESULT_OK;
      ...
      class _single_thread DGramSocketImpl : public DGramSocket
      {
        ...
        virtual u_result
            sendTo(const SocketAddress& target, const void* buffer, size_t len)
        {
          const struct sockaddr* addr =
              reinterpret_cast<const struct sockaddr*>(
                  target.getPlatformData());
          assert(addr);
          int ans = ::sendto(_socket_fd, ( const char* )buffer, ( int )len, 0,
                             addr, ( int )sizeof(sockaddr_storage));
          if (ans != SOCKET_ERROR)
          {
            assert(ans == ( int )len);
            return RESULT_OK;
        ...
        virtual u_result recvFrom(void* buf,
                                  size_t len,
                                  size_t& recv_len,
...
namespace rp
{
  namespace net
  {
    ...
    static inline int
        _socketHalFamilyToOSFamily(SocketBase::socket_family_t family)
    {
      switch (family)
      {
    ...
    DGramSocket* DGramSocket::CreateSocket(SocketBase::socket_family_t family)
    {
      ...
      int socket_fd =
          ::socket(socket_family,
                   (family == SOCKET_FAMILY_RAW) ? SOCK_RAW : SOCK_DGRAM, 0);
      if (socket_fd == -1)
        return NULL;
      DGramSocket* newborn = static_cast<DGramSocket*>(
          new rp::arch::net::DGramSocketImpl(socket_fd));
      return newborn;
    }
