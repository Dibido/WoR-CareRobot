...
namespace rp
{
  namespace net
  {
    ...
    u_result SocketAddress::getRawAddress(_u8* buffer, size_t bufferSize) const
    {
      ...
      {
      case ADDRESS_TYPE_INET:
        if (bufferSize < sizeof(in_addr_t))
    ...
    namespace net
    {
      ...
      class _single_thread StreamSocketImpl : public StreamSocket
      {
        ...
        StreamSocketImpl(int fd) : _socket_fd(fd)
        {
          ...
          ::setsockopt(_socket_fd, SOL_SOCKET, SO_NOSIGPIPE,
                       ( char* )&bool_true, sizeof(bool_true));
        ...
        virtual u_result setTimeout(_u32 timeout, socket_direction_mask msk)
        {
          ...
          {
            ans = ::setsockopt(_socket_fd, SOL_SOCKET, SO_RCVTIMEO, &tv,
                               sizeof(tv));
            if (ans)
              return RESULT_OPERATION_FAIL;
          }
          ...
          {
            ans = ::setsockopt(_socket_fd, SOL_SOCKET, SO_SNDTIMEO, &tv,
                               sizeof(tv));
            if (ans)
              return RESULT_OPERATION_FAIL;
          }
    ...
    virtual u_result recvNoWait(void *buf, size_t len, size_t & recv_len)
    {
      ...
      class _single_thread DGramSocketImpl : public DGramSocket
      {
            ...
            sendTo(const SocketAddress& target, const void* buffer, size_t len)
        {
          ...
          assert(addr);
          size_t ans = ::sendto(_socket_fd, buffer, len, 0, addr,
